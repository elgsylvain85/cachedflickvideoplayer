...
class FlickControlManager extends ChangeNotifier {
  ...
  CachedVideoPlayerController get _cachedVideoPlayerController =>
      _flickManager.flickVideoManager.cachedVideoPlayerController;
  ...
  Future<void> play() async {
    ...
    if (_isMute && _cachedVideoPlayerController.value.volume != 0) {
      _cachedVideoPlayerController.setVolume(0);
    }

    await _cachedVideoPlayerController.play();
  ...
  Future<void> seekTo(Duration moment) async {
    await _cachedVideoPlayerController.seekTo(moment);
  }
  ...
  Future<void> setVolume(double volume) async {
    await _cachedVideoPlayerController?.setVolume(volume);
    _notify();
  }
